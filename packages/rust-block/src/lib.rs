...
mod tests {
    ...
    fn test_create_block() {
        ...
        assert_eq!(unsafe { block.call(()) }, 13);
    }
    ...
    fn test_concrete_block_copy() {
        ...
        assert_eq!(unsafe { block.call(()) }, expected_len);

        let copied = block.copy();
        assert_eq!(unsafe { copied.call(()) }, expected_len);
    }
    ...
    fn test_concrete_block_stack_copy() {
        fn make_block() -> RcBlock<(), i32> {
            let x = 7;
            ConcreteBlock::new(move || x).copy()
        }

        let block = make_block();
        assert_eq!(unsafe { block.call(()) }, 7);
    }
}
